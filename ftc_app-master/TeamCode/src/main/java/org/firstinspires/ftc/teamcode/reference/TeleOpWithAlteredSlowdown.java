 ...
 */
@TeleOp(name="Driver Controlled (New Without RPM)", group="TeleOpOld")
@Disabled
public class TeleOpWithAlteredSlowdown extends OpMode {
    ...
    double convertNanoToMinutes(double nano){
        return convertSecsToMins(convertMilliToSecs(convertNanoToMilli(nano)));
    }
    double convertNanoToMilli(double nano){
        return nano/1000;
    }
    ...
    public void loop() {
        ...
        */
        ...
        lastTime = getRuntime();
        detlaShooter = lastShooter-shoot1.getCurrentPosition();
        lastShooter = shoot1.getCurrentPosition();
        detlaShooter = convertTicksToRots( detlaShooter );
        deltaTime = convertNanoToMinutes(deltaTime);
        RPM = detlaShooter / deltaTime;
