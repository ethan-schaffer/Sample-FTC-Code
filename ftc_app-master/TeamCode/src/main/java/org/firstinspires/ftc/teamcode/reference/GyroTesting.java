...
@Autonomous(name="GyroTesting", group="Autonomous")
@Disabled
public class GyroTesting extends LinearOpMode {
    ...
    state[] stateOrder = new state[]{
            //              State         Sensor       Power
            new state(states.Move,          15,        0.75),
            new state(states.TurnLeft,      45,        0.25),
            new state(states.Move,          15,        0.75),
            new state(states.TurnLeft,      45,        0.25),

            new state(states.Move,          15,      - 0.75),
            new state(states.TurnRight,     45,        0.25),
            new state(states.Move,          15,      - 0.75),
            new state(states.TurnRight,     0,         0.25),
            new state(states.Move,          15,      - 0.75),
            new state(states.TurnRight,   - 45,        0.25),
    ...
    public void runOpMode() throws InterruptedException {
        ...
        int timer = 0;
        while(gyroSensor.isCalibrating() && timer <= 3000) {
            timer++;
            if (timer % 30 < 10) {
        ...
        if(timer > 2999) {
            telemetry.addLine("Timeout on Gyro");
        }
